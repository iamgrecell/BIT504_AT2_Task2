...
public class Board {
	...
	public Board() {
	 ...
	 //TODO: initialise the cells array using ROWS and COLS constants ]
		cells = new Cell[GameMain.ROWS][GameMain.COLS]; //ROWS and COLS constants from GameMain Class
		...
		for (int row = 0; row < GameMain.ROWS; ++row) {
			for (int col = 0; col < GameMain.COLS; ++col) {
				if (cells[row][col].content == Player.Empty) {
					return false; //not a draw because there is an empty cell
				}
			}
		}
		return true; //if all cells are not empty, it is a draw
	...
	public boolean hasWon(Player thePlayer, int playerRow, int playerCol) {
		...
		if(cells[playerCol][0].content == thePlayer && cells[playerCol][1].content == thePlayer && cells[playerCol][2].content == thePlayer )
			return true; //checked if the player has 3-in-that-column
